...
function changePage(page: string | number) {
    ...
    } else if (page == 'next') { // 下一頁
        currentPageComponent.value++;
        emit('changePageEvent', currentPageComponent.value); // 告訴父元件目前頁數
    } else if (typeof page == "number") { // 點選頁數
...
const hiddenLimitPage = { // 隱藏頁數狀態下，顯示頁數調整
    min: 1,
    middle: 3,
    max: 5
}
...
<template>
    <div class="pagination flex">
        ...
        <div v-if="totalPagesComponent <= showLimitPage">
            <button v-for="page in totalPagesComponent" :key="page"
                :class="{ 'bg-blue text-white': currentPageComponent == page, 'hover:bg-blue-light': currentPageComponent != page }"
        ...
        <div v-if="totalPagesComponent > showLimitPage" class="flex flex-col">
            ...
            <div v-if="currentPageComponent <= hiddenLimitPage.middle" class="flex">
                <div v-for="page in totalPagesComponent" :key="page">
                    <button v-if="page <= hiddenLimitPage.max"
                        :class="{ 'bg-blue text-white': currentPageComponent == page, 'hover:bg-blue-light': currentPageComponent != page }"
                        class="border border-blue rounded text-blue py-3 px-5 mx-1.5  active:bg-blue active:text-white"
                        @click="changePage(page)">
                        <h6>{{ page }}</h6>
                ...
                <div v-for="page in totalPagesComponent" :key="page">
                    <button v-if="page > totalPagesComponent - hiddenLimitPage.min"
                        :class="{ 'bg-blue text-white': currentPageComponent == page, 'hover:bg-blue-light': currentPageComponent != page }"
                        class="border border-blue rounded text-blue py-3 px-5 mx-1.5  active:bg-blue active:text-white"
                        @click="changePage(page)">
                        <h6>{{ page }}</h6>
            ...
            <!-- 樣式二 : 靠近中間頁 -->
            <div v-if="currentPageComponent > hiddenLimitPage.middle && currentPageComponent <= totalPagesComponent - hiddenLimitPage.middle"
                class="flex">
                <div v-for="page in totalPagesComponent" :key="page">
                    <button v-if="page <= hiddenLimitPage.min"
                        :class="{ 'bg-blue text-white': currentPageComponent == page, 'hover:bg-blue-light': currentPageComponent != page }"
                        class="border border-blue rounded text-blue py-3 px-5 mx-1.5  active:bg-blue active:text-white"
                        @click="changePage(page)">
                        <h6>{{ page }}</h6>
            ...
            <div v-if="currentPageComponent > totalPagesComponent - hiddenLimitPage.middle" class="flex">
                ...
                <div v-for="page in totalPagesComponent" :key="page">
                    <button v-if="page > totalPagesComponent - hiddenLimitPage.max"
                        :class="{ 'bg-blue text-white': currentPageComponent == page, 'hover:bg-blue-light': currentPageComponent != page }"
                        class="border border-blue rounded text-blue py-3 px-5 mx-1.5  active:bg-blue active:text-white"
                        @click="changePage(page)">
                        <h6>{{ page }}</h6>
