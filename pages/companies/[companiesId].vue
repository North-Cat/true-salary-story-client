...
<template>
  <section class="companies">
              ...
              <div
                ...
                <div class="flex mb-6">
                  ...
                  <label v-for="item in sortOptions" :key="item.value" :for="`sort-${item.value}`"
                    ...
                    <input :id="`sort-${item.value}`" type="radio" :value="item.value" name="sort" v-model="sortValue"
                      class="appearance-none w-[20px] h-[20px] relative 
                ...
                <div class="flex mb-5">
                  ...
                  <label v-for="item in titleOptions" :key="item" :for="`title-sort-${item}`" class="me-5">
                    <input :id="`title-sort-${item}`" type="checkbox" :value="item" name="title-sort" v-model="titleValue"
    ...
    <!-- sm 篩選選單 -->
    <div ref="filterModal" v-if="showFilterModal"
      ...
      <div class="flex flex-col mb-6">
        ...
        <div>
          <label v-for="item in sortOptions" :key="item.value" :for="`sort-${item.value}`"
            ...
            <input :id="`sort-${item.value}`" type="radio" :value="item.value" name="sort" v-model="sortValue" class="appearance-none w-[20px] h-[20px] relative 
              after:content-[''] after:absolute after:top-1/2 after:translate-y-[-50%] after:right-0 after:w-[15px] after:h-[15px] after:border after:border-black-6 after:border-solid after:rounded-full
              checked:after:border-[6px] checked:after:border-blue
              " :checked="item.value === sortValue">
            <span :class="{ 'text-blue': item.value === sortValue }" class="ml-2">{{ item.text }}</span>
