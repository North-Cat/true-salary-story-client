...
useStore.tryToFetchSubscribeCompanies(searchParam).finally(() => {
  loading.value = false;
});
const search = async () => {
  loading.value = true;
  await useStore.tryToFetchSubscribeCompanies(searchParam).finally(() => {
    loading.value = false;
  });
  ...
  <userLayouts>
    <template v-if="!loading">
      <div class="flex justify-between items-center mb-4">
        ...
        <span v-if="subscribeCompaniesList.totalCount"
          >第{{ searchParam.page }}頁,共{{
            subscribeCompaniesList.totalCount || subscribeCompaniesList.result.length
          }}筆</span
        >
      </div>
      <template v-if="subscribeCompaniesList.result.length > 0 || isSearch">
        <BaseNull v-if="currentResultsList.length === 0 && isSearch" content="查無資料" />
        <template v-else>
              ...
              <nuxt-link :to="`/companies/${item.company.companyId}`" class="flex">
                ...
                <div>
                  ...
                  <div class="text-sm text-black-6">
                    ...
                    <span class="text-[0.2rem] relative bottom-[2px] px-1">|</span> 共 {{ item.company.shared }} 則
                  </div>
                </div>
              </nuxt-link>
              <div class="ml-auto">
